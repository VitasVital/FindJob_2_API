...
export class Resume extends Component{
    ...
    refreshList(){
        ...
        fetch(process.env.REACT_APP_API+'Client/GetResumesWithParams/'
            ...
            <div >
                <Table className="mt-4" striped bordered hover size="sm" style={{ textAlign: "center" }}>
                    ...
                    <tr>
                        <th>Поиск</th>
                        <td>
                            <Form.Control type="text" placeholder = "Профессия" value={this.state.inputSearch} onChange={ this.handleInputChange3 }/>
                        </td>
                    </tr>
                    <tr>
                ...
                <Table className="mt-4" striped bordered hover size="sm" style={{ textAlign: "center" }}>
                    ...
                    {deps.map(dep=>
                        <tr key={dep.id}>
                            ...
                            <td>{dep.country}</td>
                            <td>{dep.region}</td>
                            <td>{dep.salary} руб.</td>
                            <td>{dep.education}</td>
                            <td>{dep.workExperience}</td>
                            <td>{dep.employment}</td>
                            <td>{dep.workSchedule}</td>
                            <td>
                                <Route render={({ history}) => (
                                    <Button className="mr-2" variant="info" onClick={() => { history.push( '/ResumeModal/?clientId=' + dep.id) }}>
                                        Просмотр
                                    </Button>
                                )} />
                            </td>
